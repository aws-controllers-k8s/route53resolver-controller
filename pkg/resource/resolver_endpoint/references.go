...
func (rm *resourceManager) ClearResolvedReferences(res acktypes.AWSResource) acktypes.AWSResource {
	...
	for f0idx, f0iter := range ko.Spec.IPAddresses {
		if f0iter.SubnetRef != nil {
			ko.Spec.IPAddresses[f0idx].SubnetID = nil
		}
	}
...
func (rm *resourceManager) ResolveReferences(
	...
	if fieldHasReferences, err := rm.resolveReferenceForIPAddresses_SubnetID(ctx, apiReader, namespace, ko); err != nil {
...
// identifier field.
func validateReferenceFields(ko *svcapitypes.ResolverEndpoint) error {

	for _, f0iter := range ko.Spec.IPAddresses {
		if f0iter.SubnetRef != nil && f0iter.SubnetID != nil {
			return ackerr.ResourceReferenceAndIDNotSupportedFor("IPAddresses.SubnetID", "IPAddresses.SubnetRef")
		}
	}
	...
	if len(ko.Spec.SecurityGroupRefs) == 0 && len(ko.Spec.SecurityGroupIDs) == 0 {
		return ackerr.ResourceReferenceOrIDRequiredFor("SecurityGroupIDs", "SecurityGroupRefs")
	}
	return nil
}

// resolveReferenceForIPAddresses_SubnetID reads the resource referenced
// from IPAddresses.SubnetRef field and sets the IPAddresses.SubnetID
// from referenced resource. Returns a boolean indicating whether a reference
// contains references, or an error
func (rm *resourceManager) resolveReferenceForIPAddresses_SubnetID(
	ctx context.Context,
	apiReader client.Reader,
	namespace string,
	...
	for f0idx, f0iter := range ko.Spec.IPAddresses {
		if f0iter.SubnetRef != nil && f0iter.SubnetRef.From != nil {
			...
			if arr.Name == nil || *arr.Name == "" {
				return hasReferences, fmt.Errorf("provided resource reference is nil or empty: IPAddresses.SubnetRef")
			}
			obj := &ec2apitypes.Subnet{}
			if err := getReferencedResourceState_Subnet(ctx, apiReader, obj, *arr.Name, namespace); err != nil {
				return hasReferences, err
			}
			ko.Spec.IPAddresses[f0idx].SubnetID = (*string)(obj.Status.SubnetID)
		}
	}
